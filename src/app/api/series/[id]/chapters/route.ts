...
// v2.2.0 - QA fixes for NaN handling and advisory lock safety
const UUID_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i

/**
 * QA FIX: Safe integer parser with bounds checking
 */
...
export async function GET(
  ...
  try {
    ...
    // QA FIX: Use safeParseInt to prevent NaN issues
    ...
    const formattedChapters = chapters.map((c: any) => {
      ...
      // QA FIX: Handle edge case where lastReadChapter is NaN
...
async function performOnDemandSync(seriesId: string) {
  ...
  // QA FIX: Validate lockId is a valid number
  if (isNaN(lockId)) {
  ...
  try {
    ...
    // QA FIX: Safe array access with validation
    ...
    try {
      // Count chapters AND check if they have sources
      ...
      // FIX: Also sync if chapters exist but have NO sources (orphaned chapters from failed syncs)
      const hasOrphanedChapters = currentCount > 0 && sourceCount === 0;
      
      if (currentCount > 0 && !isDummy && !hasOrphanedChapters) {
        console.log(`[Sync] Series ${seriesId} already has ${currentCount} chapters with ${sourceCount} sources, skipping on-demand sync.`);
        return;
      }

      if (hasOrphanedChapters) {
        console.log(`[Sync] Series ${seriesId} has ${currentCount} orphaned chapters (no sources), forcing re-sync.`);
      }
      ...
      // QA FIX: Always release lock in finally block
